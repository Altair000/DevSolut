...
def otorgar_rango_admin(conn, user_id, duracion):
    with conn.cursor() as cursor:
        ...
        try:
            ...
            print(f"El rango del usuario con ID {user_id} ha sido actualizado a 'Admin' hasta {fecha_expiracion.strftime('%d/%m/%Y')}.")
        except Exception as e:
            print(f"Error al otorgar rango admin: {e}")
            conn.rollback()  # Revertir los cambios en caso de error

# Ejemplo de cómo llamar a esta función
def set_admin(user_id, duracion):
    conn = connect()
    if conn:
        otorgar_rango_admin(conn, user_id, duracion)
        conn.close()
